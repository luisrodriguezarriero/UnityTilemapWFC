...
namespace WFC.Solution
{
    public abstract class Solver
    {
        ...
        protected bool[][] wave;
        protected int[][][] compatible = new int[4][][];
            ...
            for (var i = 0; i < wave.Length; i++)
            {
                for (var t = 0; t < model.nTiles; t++)
                {
                    ...
                    for (var d = 0; d < 4; d++) 
                        try
                        {
                            compatible[i][t][d] = model.propagator[opposite[d]][t].Length;
                        }
                        catch(IndexOutOfRangeException e)
                        {
                            Debug.LogError(message: $"Error on {e.Source} position {d} {t}/{model.propagator[d].Length}");
                        }
                }
