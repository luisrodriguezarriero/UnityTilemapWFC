...
namespace WFC.Unity
{
    abstract public class MapGenerator : MonoBehaviour{
        ...
        protected virtual Solution.Solver initSolver(int MX, int MY, bool periodicOutput){
            switch (heuristic)
            {
                case Heuristic.Entropy:
                    return new Solution.Entropy(model, MX, MY, periodicOutput);
                case Heuristic.MRV:
                    return new Solution.MRV(model, MX, MY, periodicOutput);
                case Heuristic.Scanline:
                default: 
                    return new Solution.Scanline(model, MX, MY, periodicOutput);
            }
        }
    }
}