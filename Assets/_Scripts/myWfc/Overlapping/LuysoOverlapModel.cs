...
namespace LuysoWFC
{
    public class LuysoOverlapModel : LuysoModel
    {
        ...
        List<byte[]> patterns;
        public LuysoOverlapModel(Tilemap inputImage, Tilemap outputImage, int patternSize, int outputWidth, int outputHeight, bool periodic, int simmetry, HeuristicType heuristicType) :
            base(outputWidth, outputHeight, patternSize, periodic, heuristicType)
        {
            ...
            PatternListCreator p = new PatternListCreator(tilesAsBytes, SX, SY, N, simmetry, periodic, inputImage.GetUsedTilesCount());
        ...
        protected override void InitPropagator()
        {
            ...
            if (heuristic.observed[0] >= 0)
            {
                for (int y = 0; y < MY; y++)
                {
                    ...
                    for (int x = 0; x < MX; x++)
                    {
                        ...
                        outputImage.SetTile(new Vector3Int(x, y, 0), tiles[patterns[heuristic.observed[x - dx + (y - dy) * MX]][dx + dy * N]]);
                    }
                }
            }
            Debug.Log(ToString());
        ...
        protected override bool isNodeExcluded(int n)
        {
